...
{
	if ([self _isValidDelegateForSelector:@selector(twitterRequestFailed:withError:)])
		[_delegate twitterRequestFailed:requestIdentifier withError:error];
}
...
- (void)connection:(MGTwitterHTTPURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
  ...
  if (statusCode >= 400) {
    ...
    NSError *error = [NSError errorWithDomain:@"HTTP" code:statusCode userInfo:nil];
		if ([self _isValidDelegateForSelector:@selector(twitterRequestFailed:withError:)])
			[_delegate twitterRequestFailed:[connection identifier] withError:error];
    ...
    // Not modified, or generic success.
		if ([self _isValidDelegateForSelector:@selector(twitterRequestSucceeded:)])
			[_delegate twitterRequestSucceeded:[connection identifier]];
    if (statusCode == 304) {
